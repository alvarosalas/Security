...
function NinjaSlider(e) {
	var t = document,
		...
		h, d, a, g, n, b, m, k, Y, I, cb, z, w, E, j, S, s, D, v, A, H, q, p, kb, xb, jb, L = (navigator.msPointerEnabled || navigator.pointerEnabled) && navigator.msMaxTouchPoints,
		N, F, G, gb = function (a) {
		...
		Lb = function (b) {
			if (!b.d) {
				...
				a.i = new Image;
				a.i.s = null;
				if (b[y][f]) b.insertBefore(a, b[y][0]);
				else b[X](a);
				b.d = a;
		...
		ub = function (B) {
			...
			if (u) n = {
				...
				a: function () {
					...
					for (var b in a)
						if (h[c][a[b] + "ransition"] !== undefined) return true;
					return false
				}()
			};
			if (n.c)
	...
	function sb(d, f, e) {
		for (var a = [], c = Math.ceil(e / 16), b = 1; b <= c; b++)
			if (k) a.push(b / c);
			else a.push(Math.round(d + Ub(b / c) * (f - d)));
		a.a = 0;
		return a
	}
	...
	var Qb = function (c) {
			...
			if (window[o]) window[o]("load", a, false);
			else window.attachEvent && window.attachEvent("onload", a)
		},
		vb = function () {
			var a = U(e.sliderId);
			if (a && a[y][f] && a.offsetWidth) ub(0);
			else r(vb, 90)
		};
	Qb(vb);
	return {
